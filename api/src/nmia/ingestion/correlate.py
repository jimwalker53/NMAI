"""Identity correlation: link identities to systems based on SANs, SPNs, and
naming patterns.
"""
...
def _extract_host_from_spn(spn: str) -> str | None:
    ...
    if "/" in spn:
        parts = spn.split("/", 1)
        host = parts[1]
        # Strip port if present (service/host:port)
        if ":" in host:
            host = host.split(":")[0]
...
def correlate_identities(
    ...
    """Correlate identities to systems based on SANs, SPNs, and naming patterns.

    Returns the number of identities correlated.
    """
    ...
    for identity in identities:
        ...
        if identity.identity_type == "cert":
                ...
                if subject_dn:
                    ...
                    for part in subject_dn.split(","):
                        ...
                        if part.upper().startswith("CN="):
                            ...
                            # Check for hostname.domain pattern (contains at least one dot)
                            if "." in cn_value:
                                linked = cn_value
                            break
            ...
            if isinstance(spn_list, list) and spn_list:
                ...
                first_spn = spn_list[0] if isinstance(spn_list[0], str) else str(spn_list[0])
